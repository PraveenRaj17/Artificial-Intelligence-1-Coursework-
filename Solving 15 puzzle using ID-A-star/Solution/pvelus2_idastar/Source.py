...
def IDA_star(root, h_type, goal):
    ...
    path.append(root)
    NExp = [] #List of nodes expanded
    while True:
        t, p, nExpanded, mem_used = a_star(path, goal, bound, h_type, NExp)
        if t == "FOUND" : 
            return bound, p, t, nExpanded, mem_used
        if t == float("inf"): return "NOT FOUND", None, None, None, None
        bound = t
        # # ---------------------------------------------------------------------------------------------


def a_star(path, goal_state, limit, heuristic_type, NodesExpanded):
    
    node = path[-1]
    f = eval_function(node, heuristic_type, goal_state)
    if f > limit: return f, None, None, None
    if node.state.tiles == goal_state : 
        return "FOUND", find_path(node), len(NodesExpanded), max(0, sys.getsizeof(NodesExpanded))
    min = float("inf")
    ...
    for child in children_nodes :
        if child not in path:
            path.append(child)
            t, nd, nNExpanded, memo = a_star(path, goal_state, limit, heuristic_type, NodesExpanded)
            if t == "FOUND": return "FOUND", nd, nNExpanded, memo
            if t < min: min = t
            path.pop()
...
def main():
    ...
    for heuristic in heuristics:
        ...
        r, pth, st, num_NodesExpanded, m_used = IDA_star(root, heuristic, goal_state)
        end_time = time.time()

        print("Moves: " , pth)
        print("Number of expanded Nodes: "+ str(num_NodesExpanded))
        print("Time Taken: " + str(end_time - start_time))
        print("Max Memory (Bytes): " +  str(m_used))
